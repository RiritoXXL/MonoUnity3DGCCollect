...
typedef void(*t_mono_gc_collect)(int collect);
...
private:
	...
	t_mono_gc_collect mono_gc_collect = nullptr;
	...
	void Initalize()
	{
		...
		mono_gc_collect = reinterpret_cast<t_mono_gc_collect>(GetProcAddress(hMono, "mono_gc_collect"));
...
public:
	static MonoUnity& Instance_DLL() // Singleton time!
	{
		static MonoUnity _instance;
	...
	void GCCollect(int generate_collect) {
		MonoDomain* RootDomain = mono_get_root_domain();
		MonoThread* Thread = mono_thread_attach(RootDomain);
		if (!Thread) {
			printf("Failed to get Mono Thread!!!");
		}
		mono_gc_collect(generate_collect);
	}
};
